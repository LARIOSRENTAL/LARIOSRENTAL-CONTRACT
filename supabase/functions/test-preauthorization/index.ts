...
// Separate TEST service: never use production contracts, tokens or live keys.
const env=n=>(Deno.env.get(n)||'').trim();
const key=()=>{const k=env('STRIPE_TEST_SECRET_KEY')||env('STRIPE_SECRET_KEY');return /^(sk|rk)_test_/.test(k)?k:''};
const loc=()=>env('STRIPE_TEST_TERMINAL_LOCATION_ID');
const caps=()=>({environment:'test',stripe_configured:!!key(),terminal_configured:!!key()&&/^tml_[A-Za-z0-9_]+$/.test(loc()),tap_native_required:true});
...
async function stripe(path,p=null,idem=''){
 if(!key())throw Error('Falta la clave Stripe de PRUEBAS. Las claves reales estan bloqueadas.');
 ...
 const d=await r.json();if(!r.ok)throw Error(d.error?.message||'Stripe no ha confirmado la operacion');if(d.livemode===true)throw Error('Respuesta REAL bloqueada');return d;
}
function verify(pi,h){if(pi.livemode!==false||pi.capture_method!=='manual'||pi.currency!=='eur'||Number(pi.amount)!==Number(h.amount_cents)||pi.metadata?.hold_id!==h.id||pi.metadata?.contract_id!==h.contract_id||pi.metadata?.environment!=='larios_test'||pi.metadata?.purpose!=='rental_guarantee'||(h.payment_intent_id&&pi.id!==h.payment_intent_id))throw Error('La retencion no corresponde a esta garantia de pruebas');return pi}
...
function parameters(h){const p=new URLSearchParams(),meta={contract_id:h.contract_id,hold_id:h.id,environment:'larios_test',purpose:'rental_guarantee'};if(h.channel==='stripe_checkout'){
 ...
 p.set('line_items[0][quantity]','1');p.set('line_items[0][price_data][currency]','eur');p.set('line_items[0][price_data][unit_amount]',String(amount(Number(h.amount_cents))));p.set('line_items[0][price_data][product_data][name]','PRUEBAS - Garantia Larios Rental - solo retencion');
 ...
 const url='https://lariosrental.github.io/LARIOSRENTAL-CONTRACT/pruebas/?hold_return='+h.id+'&contract_id='+h.contract_id;p.set('success_url',url);p.set('cancel_url',url+'&hold_canceled=1');
 }else{p.set('amount',String(amount(Number(h.amount_cents))));p.set('currency','eur');p.set('payment_method_types[0]','card_present');p.set('capture_method','manual');p.set('description','PRUEBAS - Garantia - solo retencion');for(const [k,v] of Object.entries(meta))p.set(`metadata[${k}]`,v)}return p}
Deno.serve(async req=>{
 ...
 const b=await req.json().catch(()=>null);if(!b||b.environment!=='test')return reply({error:'Solo PRUEBAS'},400);
 const ref=h=>({hold_id:h.id,contract_id:h.contract_id});
 async function command(a,d){const {data,error}=await db.rpc('test_preauth_command',{p_action:a,p_data:{...d,actor_id:auth.user.id}});if(error)throw Error(error.message);return data}
 async function reader(id){if(!caps().terminal_configured||!/^tmr_[A-Za-z0-9_]+$/.test(id||''))throw Error('Falta lector o ubicacion de PRUEBAS');const r=await stripe('terminal/readers/'+id);if(r.location!==loc())throw Error('Lector fuera de la ubicacion de PRUEBAS');return r}
 async function refresh(h){
  if(h.channel==='bank'||terminalStates.includes(h.status))return h;
  if(h.session_id){const s=await stripe('checkout/sessions/'+h.session_id);if(s.livemode!==false||s.client_reference_id!==h.contract_id||s.metadata?.hold_id!==h.id)throw Error('Sesion incorrecta');if(s.payment_intent&&!h.payment_intent_id){const id=String(s.payment_intent);if(!/^pi_[A-Za-z0-9_]+$/.test(id))throw Error('Referencia Stripe no valida');h=await command('link',{...ref(h),payment_intent_id:id})}if(s.status==='expired'&&!s.payment_intent)return command('sync',{...ref(h),status:'expired',released_cents:0})}
 ...
 try{
  if(b.action==='status'){
   if(!b.contract_id)return reply(caps());if(!validId(b.contract_id))throw Error('Reserva no valida');const {data:c,error}=await db.from('test_contracts').select('id,category').eq('id',b.contract_id).single();if(error||!c)throw Error('Reserva de PRUEBAS no encontrada');
   const {data:holds,error:e}=await db.from('test_preauthorizations').select('*').eq('contract_id',c.id).order('created_at',{ascending:false});if(e)throw Error(e.message);const {data:events,error:ee}=await db.from('test_preauthorization_events').select('event,detail,created_at').eq('contract_id',c.id).order('created_at',{ascending:false}).limit(40);if(ee)throw Error(ee.message);return reply({...caps(),category:c.category,holds,events});
  }
  if(b.action==='readers'){if(!caps().terminal_configured)throw Error('Falta la ubicacion de Terminal de PRUEBAS');const r=await stripe('terminal/readers?limit=100&location='+loc());return reply({readers:r.data.filter(x=>x.location===loc()).map(x=>({id:x.id,label:x.label,status:x.status}))})}
  if(b.action==='connection_token'){if(!caps().terminal_configured)throw Error('Falta configuracion Terminal TEST');return reply(await stripe('terminal/connection_tokens',new URLSearchParams({location:loc()})))}
  if(b.action==='create'){
   if(!validId(b.contract_id)||!['stripe_checkout','stripe_terminal','stripe_tap','bank'].includes(b.channel)||b.confirmed!==true)throw Error('Confirma la preautorizacion');if(b.channel!=='bank'&&!key())throw Error('Stripe de PRUEBAS no configurado. No se ha retenido ni cobrado dinero.');
   if(b.channel==='stripe_terminal'){const r=await reader(b.reader_id);if(r.status!=='online')throw Error('Lector desconectado')}
   if(b.channel==='stripe_tap'&&!caps().terminal_configured)throw Error('Tap to Pay de PRUEBAS no configurado');
   ...
   if(h.channel==='stripe_checkout'){const s=await stripe('checkout/sessions',parameters(h),'lr-test-hold-'+h.id+'-create');if(s.livemode!==false)throw Error('Sesion no reconocida como TEST');h=await command('link',{...ref(h),session_id:s.id,checkout_url:s.url})}
   else{const pi=verify(await stripe('payment_intents',parameters(h),'lr-test-hold-'+h.id+'-create'),h);h=await command('link',{...ref(h),payment_intent_id:pi.id,reader_id:b.reader_id||null});if(h.channel==='stripe_tap')return reply({hold:h,client_secret:pi.client_secret,...caps()})}
   return reply({hold:h,...caps()});
  }
  if(!validId(b.hold_id)||!validId(b.contract_id))throw Error('Referencia no valida');const {data:found,error}=await db.from('test_preauthorizations').select('*').eq('id',b.hold_id).eq('contract_id',b.contract_id).single();if(error||!found)throw Error('Preautorizacion de PRUEBAS no encontrada');let h=found;
  ...
  if(b.action==='process_terminal'){
   ...
   if(r.action?.status!=='in_progress')await stripe('terminal/readers/'+h.reader_id+'/process_payment_intent',new URLSearchParams({payment_intent:h.payment_intent_id,'process_config[enable_customer_cancellation]':'true'}),'lr-test-hold-'+h.id+'-process');return reply({hold:h});
  }
  ...
  if(b.action==='capture'){
   ...
   const done=await stripe('payment_intents/'+h.payment_intent_id+'/capture',new URLSearchParams({amount_to_capture:String(n),final_capture:'true'}),'lr-test-hold-'+h.id+'-capture-'+n);h=await command('sync',{...ref(h),...result(done,h)});
  }else{
   if(h.session_id){let s=await stripe('checkout/sessions/'+h.session_id);if(s.livemode!==false||s.client_reference_id!==h.contract_id||s.metadata?.hold_id!==h.id)throw Error('Sesion incorrecta');if(s.status==='open')await stripe('checkout/sessions/'+h.session_id+'/expire',new URLSearchParams(),'lr-test-hold-'+h.id+'-expire');s=await stripe('checkout/sessions/'+h.session_id);if(s.payment_intent&&!h.payment_intent_id){h=await command('link',{...ref(h),payment_intent_id:String(s.payment_intent)})}}
   if(h.reader_id){const r=await reader(h.reader_id);if(r.action?.status==='in_progress'&&r.action?.process_payment_intent?.payment_intent===h.payment_intent_id)await stripe('terminal/readers/'+h.reader_id+'/cancel_action',new URLSearchParams(),'lr-test-hold-'+h.id+'-reader-cancel')}
   if(h.payment_intent_id){let pi=verify(await stripe('payment_intents/'+h.payment_intent_id+'?expand[]=latest_charge'),h);if(!['succeeded','canceled'].includes(pi.status))pi=await stripe('payment_intents/'+h.payment_intent_id+'/cancel',new URLSearchParams({cancellation_reason:'requested_by_customer'}),'lr-test-hold-'+h.id+'-cancel');h=await command('sync',{...ref(h),...result(pi,h)})}
  ...
  return reply({hold:h,...caps()});
 }catch(e){return reply({error:e instanceof Error?e.message:String(e),environment:'test',confirmed:false},409)}
});
