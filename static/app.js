...
let scanReviewState = { scanType: "", fields: [], files: [] };
...
function detectOcrCrop(ctx, width, height) {
...
async function prepareOcrImages(file) {
  ...
  try {
    ...
    const crop = detectOcrCrop(ctx, canvas.width, canvas.height);
    const croppedCanvas = document.createElement("canvas");
    const cropRatio = targetWidth / crop.width;
    croppedCanvas.width = targetWidth;
    croppedCanvas.height = Math.max(1, Math.round(crop.height * cropRatio));
    const croppedCtx = croppedCanvas.getContext("2d", { alpha: false });
    croppedCtx.fillStyle = "#fff";
    croppedCtx.fillRect(0, 0, croppedCanvas.width, croppedCanvas.height);
    croppedCtx.imageSmoothingEnabled = true;
    croppedCtx.imageSmoothingQuality = "high";
    croppedCtx.drawImage(canvas, crop.x, crop.y, crop.width, crop.height, 0, 0, croppedCanvas.width, croppedCanvas.height);

    const softCanvas = document.createElement("canvas");
    softCanvas.width = croppedCanvas.width;
    softCanvas.height = croppedCanvas.height;
    const softCtx = softCanvas.getContext("2d", { alpha: false });
    softCtx.drawImage(croppedCanvas, 0, 0);
    ...
    hardCanvas.width = croppedCanvas.width;
    hardCanvas.height = croppedCanvas.height;
    const hardCtx = hardCanvas.getContext("2d", { alpha: false });
    hardCtx.drawImage(croppedCanvas, 0, 0);
    ...
    return [softCanvas.toDataURL("image/png"), hardCanvas.toDataURL("image/png")];
  } finally {
  ...
  try {
    ...
    for (let index = 0; index < scanReviewState.files.length; index += 1) {
      ...
      try {
        ...
        const image = await fileToDataUrl(scanReviewState.files[index]);
        const result = await postJson("/api/vision-ocr", { image, scan_type: scanReviewState.scanType });
        nextFields = result.fields || {};
      } catch (_serverError) {
        if (!(await ensureBrowserOcr())) throw new Error("No se pudo cargar el lector OCR.");
        setStatus(`Mejorando imagen ${index + 1}/${scanReviewState.files.length}...`);
        const images = await prepareOcrImages(scanReviewState.files[index]);
        let text = await readOcrImage(images[0], `${index + 1}/${scanReviewState.files.length}`);
        nextFields = parseOcrFields(text, scanReviewState.scanType);
        if (Object.keys(nextFields).length < 2) {
          const fallbackText = await readOcrImage(images[1], `${index + 1}/${scanReviewState.files.length} extra`);
...
function openScanReview(scanType, imageUrl, files = []) {
  ...
  scanReviewState = { scanType, fields: config.fields.map(([name]) => name), files };
...
function closeScanReview() {
  ...
  scanReviewFields.replaceChildren();
  scanReviewState = { scanType: "", fields: [], files: [] };
}
