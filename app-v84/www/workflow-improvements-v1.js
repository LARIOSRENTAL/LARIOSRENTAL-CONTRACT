...
function boot(){ensureCashBox();const section=$('reservation');if(section)new MutationObserver(()=>{if(!section.classList.contains('hidden'))setTimeout(hydrate,80)}).observe(section,{attributes:true,attributeFilter:['class']});const agenda=$('agenda');if(agenda)new MutationObserver(scheduleAgenda).observe(agenda,{childList:true,subtree:true});const form=$('reservationForm');if(form)new MutationObserver(()=>{ensureCashBox();if(!$('reservation')?.classList.contains('hidden'))setTimeout(hydrate,20)}).observe(form,{childList:true,subtree:true});scheduleAgenda();const q=new URLSearchParams(location.search),target=q.get('contract_id')||sessionStorage.getItem('lr_resume_contract');if(target)setTimeout(()=>resumeContract(target),700)}
const priorFetch=window.fetch.bind(window);window.fetch=async function(input,init){try{const url=typeof input==='string'?input:String(input?.url||'');if(/\/rest\/v1\/rpc\/(?:test_)?app_save_contract(?:\?|$)/.test(url)&&init?.body){const body=JSON.parse(init.body),p=body?.p_payload;if(p&&p.id&&p.id===window.LariosCurrentContractId){p.cash_without_card=!!$('cash_without_card')?.checked;p.agency=$('agency')?.value||p.agency||'';init={...init,body:JSON.stringify(body)}}}}catch(e){console.warn('Workflow payload',e)}return priorFetch(input,init)};
