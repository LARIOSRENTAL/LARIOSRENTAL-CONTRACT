...
const baseV3=window.LariosQuickReservationV3;
if(!baseV3)return;
...
function normalizeScooter(raw){const info=scooterInfo(raw);if(!info)return raw;return String(raw).replace(/\b\d+\s*x\s*(?:50|125)(?:\s*cc)?\b/i,info.quantity+'x'+info.group).replace(/\b\d+\s+(?:motos?\s+)?(?:50|125)(?:\s*cc)?\b/i,info.quantity+'x'+info.group)}
function parseQuick(raw){
 let text=String(raw||'').trim();
 const isNow=/\bahora\b/i.test(text);
 if(isNow)text=text.replace(/\bahora\b/ig,madridTime());
 text=normalizeScooter(text);
 const p=baseV3.parseQuick(text);
