...
function normalizeScooter(raw){const info=scooterInfo(raw);if(!info)return raw;let text=String(raw);text=text.replace(/\b(\d+)\s*x\s*(50|125)(?:\s*cc)?\b/i,info.quantity+'x'+info.group);text=text.replace(/\b(\d+)\s+(?:motos?\s+)?(50|125)(?:\s*cc)?\b/i,info.quantity+'x'+info.group);return text}
