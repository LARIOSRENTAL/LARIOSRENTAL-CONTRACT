...
function normalizeField(el){if(!el||!el.value)return false;const parsed=iso(el.value,el.id);if(!parsed||!valid(parsed,el.id))return false;el.value=fromIso(parsed);clearError(el);return true}
...
function onDateInput(e){
  ...
  if(parsed&&(isExternalInput(e,raw)||raw.replace(/\D/g,'').length===8)){
...
function install(){configureAll();normalizeAll();patchValidation();const form=$('reservationForm');if(form&&!form.dataset.dateV2){form.dataset.dateV2='1';form.addEventListener('submit',normalizeAll,true)}console.log('Date validation V2 robust scanner-safe installed')}
