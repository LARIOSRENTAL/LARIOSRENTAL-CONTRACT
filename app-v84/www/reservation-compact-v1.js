...
function syncDepositState(){const old=$('deposit'),cashSel=$('deposit_cash_selected'),preSel=$('preauth_selected'),cash=$('deposit_cash'),pre=$('preauthorization'),fr=$('franchise');if(!old||!cashSel||!preSel||!cash||!pre)return;const scooter=isScooter(),franchise=money(fr?.value||0);if(scooter){setValue(cash,franchise);setValue(pre,franchise);setReadOnly(cash,true);setReadOnly(pre,true)}else{setReadOnly(cash,!cashSel.checked);setReadOnly(pre,!preSel.checked);if(cashSel.checked&&(!cash.value||Number(String(cash.value).replace(',','.'))===0))cash.value=franchise;if(preSel.checked&&(!pre.value||Number(String(pre.value).replace(',','.'))===0))pre.value=franchise}setValue(old,cashSel.checked?money(cash.value):preSel.checked?money(pre.value):'0.00');const hint=$('lrDepositHint');if(hint){const text=scooter?'En motos de 50cc y 125cc es obligatorio seleccionar Depósito efectivo o Preautorización. El importe se iguala a la franquicia.':'Opcional para coches, bicicletas y e-bike.';if(hint.textContent!==text)hint.textContent=text;toggleClass(hint,'lrDepositRequired',scooter&&!cashSel.checked&&!preSel.checked)}}
