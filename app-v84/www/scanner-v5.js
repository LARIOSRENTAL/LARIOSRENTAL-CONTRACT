...
function showResult(r){const b=$('scanResult');if(!b)return;if(!r||!Object.keys(r).length){b.innerHTML='<b>Sin datos fiables</b><div class="muted" style="margin-top:5px">No se copiará nada al contrato.</div>';return;}const map=[['Nombre','name'],['Documento','document'],['Permiso','license'],['Nacimiento','birth'],['Expedición','issue'],['Caducidad','expiry'],['País','country'],['Domicilio','address'],['Matrícula','plate'],['Marca','make'],['Modelo','model'],['Combustible','fuel']];b.innerHTML='<b>Datos detectados para revisar</b>'+map.filter(([l,k])=>r[k]).map(([l,k])=>`<div style="display:grid;grid-template-columns:120px 1fr;gap:8px;padding:5px 0"><strong>${l}</strong><span>${esc(r[k])}</span></div>`).join('');}
...
function vehicleValues(text,labels){const values=[],ls=String(text||'').replace(/\r/g,'').split('\n').map(clean).filter(Boolean),anyLabel=/(?:MATR[IÍ]CULA|PLATE|REGISTRATION|MARCA|MAKE|BRAND|MODELO|MODEL|FUEL|COMBUSTIBLE|CARBURANTE)\s*[:;=._-]*/i;for(let i=0;i<ls.length;i++){for(const label of labels){const same=ls[i].match(new RegExp('(?:^|[^A-Z0-9])'+label+'\\s*[:;=._-]*\\s*(.+)$','i'));if(same&&same[1]){const value=clean(same[1]).split(/\s+[|:;)\\\\]\s*/)[0].trim();if(value)values.push(value);continue;}if(new RegExp('(?:^|[^A-Z0-9])'+label+'\\s*[:;=._-]*$','i').test(ls[i])){for(const j of [i-1,i+1]){const candidate=clean(ls[j]||'');if(candidate&&!anyLabel.test(candidate)&&candidate.length<=40)values.push(candidate);}}}}return[...new Set(values)];}
...
function rawPlateCandidates(text){const labelled=vehicleValues(text,['MATR[IÍ]CULA','PLATE','REGISTRATION']),direct=upper(text).match(/\d{4}\s*[A-Z0-9]{3,4}/g)||[];return[...new Set([...labelled,...direct].map(v=>upper(v).replace(/[^A-Z0-9]/g,'')).filter(v=>v.length>=6&&v.length<=10))];}
...
function bestFleetVehicle(rows,text,result){const candidates=[result.plate,...rawPlateCandidates(text)].filter(Boolean);if(!candidates.length)return null;let best=null,bestDistance=99,ties=0;for(const vehicle of rows||[]){const registered=upper(vehicle.registration).replace(/[^A-Z0-9]/g,'');if(!registered)continue;const distance=Math.min(...candidates.map(candidate=>editDistance(candidate,registered)));if(distance<bestDistance){best=vehicle;bestDistance=distance;ties=1;}else if(distance===bestDistance){ties++;}}return bestDistance<=2&&ties===1?best:null;}
...
async function readVehicle(file){
  ...
  try{const rows=await api('vehicles?select=registration,make,model,fuel_type&limit=500');const v=bestFleetVehicle(rows,text,result);if(v){result.plate=v.registration||result.plate;result.make=v.make||result.make;result.model=v.model||result.model;result.fuel=v.fuel_type||result.fuel;}}catch(e){console.warn('No se pudo contrastar el llavero con la flota',e);}
