...
function vehicleValue(text,labels){const ls=String(text||'').replace(/\r/g,'').split('\n');for(const line of ls){for(const label of labels){const hit=line.match(new RegExp('(?:^|[^A-Z0-9])'+label+'\\s*[:;=._-]*\\s*(.+)$','i'));if(hit&&hit[1])return clean(hit[1]).split(/\s+[|:;)\\\\]\s*/)[0].trim();}}return'';}
...
function parseVehicleText(text){const plate=normalizePlate(vehicleValue(text,['MATR[IÍ]CULA','PLATE','REGISTRATION']));const make=upper(vehicleValue(text,['MARCA','MAKE','BRAND'])).replace(/[^A-Z0-9ÁÉÍÓÚÜÑ .\/-]/g,'').replace(/[.]+$/,'').trim();const model=upper(vehicleValue(text,['MODELO','MODEL'])).replace(/[^A-Z0-9ÁÉÍÓÚÜÑ .\/-]/g,'').replace(/[.]+$/,'').trim();const fuel=normalizeFuel(vehicleValue(text,['FUEL','COMBUSTIBLE','CARBURANTE']));const result={};if(plate)result.plate=plate;if(make)result.make=make;if(model)result.model=model;if(fuel)result.fuel=fuel;return result;}
...
async function readVehicle(file){
  const ocr=await getEngine(),image=await vehicleCanvas(file);state('Leyendo MATRÍCULA, MARCA, MODELO y FUEL…');await ocr.setParameters({tessedit_pageseg_mode:'6',preserve_interword_spaces:'1',user_defined_dpi:'300'});
  const r=await ocr.recognize(image),text=String(r&&r.data&&r.data.text||'').trim();$('scanRaw').textContent=text||'(sin texto)';if(!text)throw new Error('El OCR no detectó texto en el llavero');
  const result=parseVehicleText(text);if(!result.plate||!result.make||!result.model||!result.fuel)throw new Error('No se reconocieron las cuatro etiquetas MATRÍCULA, MARCA, MODELO y FUEL');
  try{const rows=await api('vehicles?select=registration,make,model,fuel_type,category,color&registration=ilike.'+encodeURIComponent(result.plate)+'&limit=1');const v=rows&&rows[0];if(v){result.plate=v.registration||result.plate;result.make=v.make||result.make;result.model=v.model||result.model;result.fuel=v.fuel_type||result.fuel;result.group=v.category||'';result.color=v.color||'';}}catch(e){console.warn('No se pudo contrastar el llavero con la flota',e);}
  return result;
}
